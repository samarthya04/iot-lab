...
TILT_ANGLE_THRESHOLD = 15  # Threshold angle for head tilt detection
...
def draw_landmarks(image, outputs, land_mark, color):
    ...
    # Get landmark points for head tilt detection
    left_eye = outputs.multi_face_landmarks[0].landmark[33]  # Left eye outer corner
    right_eye = outputs.multi_face_landmarks[0].landmark[133]  # Right eye outer corner
    nose_tip = outputs.multi_face_landmarks[0].landmark[5]  # Nose tip

    left_eye_point = (int(left_eye.x * width), int(left_eye.y * height))
    right_eye_point = (int(right_eye.x * width), int(right_eye.y * height))
    ...
    down_head_tilt = calculate_angle(left_eye_point, right_eye_point, (nose_tip_point[0], left_eye_point[1]))

    # Determine head tilt direction
    if abs(left_head_tilt) > TILT_ANGLE_THRESHOLD:
        cv.putText(image, "Left Tilt", (int(0.02 * image.shape[1]), int(0.1 * image.shape[0])),
                   cv.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 4)
    elif abs(right_head_tilt) > TILT_ANGLE_THRESHOLD:
        cv.putText(image, "Right Tilt", (int(0.02 * image.shape[1]), int(0.1 * image.shape[0])),
                   cv.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 4)
    elif abs(up_head_tilt) > TILT_ANGLE_THRESHOLD:
        cv.putText(image, "Up Tilt", (int(0.02 * image.shape[1]), int(0.1 * image.shape[0])),
                   cv.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 4)
    elif abs(down_head_tilt) > TILT_ANGLE_THRESHOLD:
        cv.putText(image, "Down Tilt", (int(0.02 * image.shape[1]), int(0.1 * image.shape[0])),
                   cv.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 4)
...
while True:
    ...
    if result:
        ...
        cv.putText(image, "Blink Count: %.2f" % (blink_counter), (int(0.02 * image.shape[1]), int(0.6 * image.shape[0])),
                   cv.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 4)
